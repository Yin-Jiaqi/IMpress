...
//#define Q 163841
ap_uint<512> Q = "52374250506775412587080182017685909013279339260195121351951847958786555732255090462694066661827009813312276859354987266719224819790981416185422168457217"; // (2^64-2^50)^4+1
...
ap_uint<2048> multiplication_1024(ap_uint<1024> x, ap_uint<1024> y){
    ap_uint<2048> result = x * y;
    return result;
}

// inner multiplication unit 2 (replace this unit 2 with the optimized multiplier)
ap_uint<1024> multiplication_512(ap_uint<512> x, ap_uint<512> y){
    ap_uint<1024> result = x * y;
    return result;
}
 ...
 */
void bitrev_shuffle(ap_int<512> *a, ap_uint<512> n) {
  ap_uint<32> i, j, k;
  ap_int<512> x;
 ...
 */
void shuffle_with_table(ap_int<512> *a, const ap_uint<512> p[][2], ap_uint<512> n) {
  ap_uint<32> i, j, k;
  ap_int<512> x;
 ...
 */
static inline ap_int<1024> sub_mod(ap_int<1024> x, ap_int<1024> y) {

  ap_int<1024> r1 = (x - y) + Q;
  ap_int<1024> r2 = (x - y);
  ap_int<1024> res = (x - y) < 0 ? r1 : r2;
  assert(0<=res<Q);
  return res; 

  //std::cout << "x: " << x  << " y: " << y << "\n";
  //x -= y;
  //std::cout << " res: " << (x < 0 ? (ap_int<512>)(x + Q) : x) << "\n";
  // return x < 0 ? (ap_int<512>)(x + Q) : x;
  // return x + ((x >> 31) & Q); // x + Q * sign(x)
  //return x + ((x >> 14) & Q); // shift by 14 is enough
}

static inline ap_int<1024> add_mod(ap_int<1024> x, ap_int<1024> y) {
  ap_int<1024> r1 = (x + y) - Q;
  ap_int<1024> r2 = (x + y);
  ap_int<1024> res = (x + y) >= Q ? r1 : r2;
  assert(0<=res<Q);
  return res; 
  //x += y;
  //return x - Q >= 0 ? (ap_int<512>)(x - Q) : x;
  //x += y - Q;
  //return x + ((x >> 14) & Q);
}
 ...
 */
static inline ap_uint<1024> divq(ap_int<1024> x) {
  return multiplication_1024(x,"209496995783604555531045821396103118003227338503580001329650653043847119641592731339048731568541382678822228862390710833850959155081118836988764068315139") >> 1010;
}

static inline ap_int<512> modq(ap_int<1024> x) {
  ap_int<513> res1 = (ap_int<513>)((ap_int<513>)x - (ap_int<1025>)(multiplication_1024(divq(x), Q)));
  ap_int<512> res2 = (res1 > Q) ? (ap_int<512>)(res1 - Q) : (ap_int<512>)res1;
  return res2; 
}
 ...
 */
void mul_array16(ap_int<512> *a, ap_uint<512> n, const ap_uint<512> *p) {
  ap_uint<32> i;

  for (i=0; i<n; i++) {
    a[i] = modq((ap_int<1024>)a[i] * p[i]);
  }
}
 ...
 */
void mul_array(ap_int<512> *c, ap_uint<512> n, ap_int<512> *a, ap_int<512> *b) {
  ap_uint<32> i;

  for (i=0; i<n; i++) {
    c[i] = modq(multiplication_512(a[i],b[i]));
    /*std::cout << "a[" << i << "]=" << a[i] << "  ";
    std::cout << "b[" << i << "]=" << b[i] << "  ";
    std::cout << "c[" << i << "]=" << c[i] << "  ";
    std::cout << "\n";*/
 ...
 */
void scalar_mul_array(ap_int<512> *a, ap_uint<512> n, ap_int<512> c) {
  ap_uint<32> i;

  for (i=0; i<n; i++) {
    a[i] = modq(multiplication_512(a[i], c));
  }
}
 ...
 */
void ntt_ct_rev2std_v1(ap_int<512> *a, ap_uint<512> n, const ap_uint<512> *p) {
  ap_uint<32> j, s, t, l;
  ap_int<512> x, w;
  ...
  for (t=1, l = n; t<n; t <<= 1, l >>= 1) {
    ...
    for (j=1; j<t; j++) {
      ...
      for (s=j; s<n; s += t + t) {
        //	x = (a[s + t] * w) % Q;
        x = modq(multiplication_512(a[s + t], w));
        a[s + t] = sub_mod(a[s], x);
        a[s] = add_mod(a[s], x);
      }
    }    
 ...
 * - output: a contains the NTT(a) in standard order
 */
void ntt_ct_rev2std(ap_int<512> *a, ap_uint<512> n, const ap_uint<512> *p) {
  ap_uint<32> j, s, t;
  ap_int<512> x, w;
 ...
 */
void ntt_ct_std2rev(ap_int<512> *a, ap_uint<512> n, const ap_uint<512> *p) {
  ...
  for (t=1; t<n; t <<= 1) {
    ...
    for (j=1; j<t; j++) {
      ...
      u += 2 * d;   // u = 2 * d * j
      for (s=u; s<u+d; s++) {
        x = modq(multiplication_512(a[s + d], w));
        a[s + d] = sub_mod(a[s], x);
        a[s] = add_mod(a[s], x);
      }
    }
